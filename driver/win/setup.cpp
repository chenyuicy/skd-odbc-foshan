...
#define INI_SERVER          TEXT("Server")      /* Name of Server  running the Skm service */
...
#define INI_PORT            TEXT("Port")        /* Port on which the Skm is listening */
...
void getDSNinfo(ConnInfo *ci, bool overwrite)
{
    ...
    if (ci->timeout[0] == '\0' || overwrite)
        SQLGetPrivateProfileString(DSN, INI_TIMEOUT, TEXT("90"), ci->timeout, sizeof(ci->timeout), ODBC_INI);	//rox //original 30
}
