...
void Connection::init() {
    ...
    //session->setTimeout(Poco::Timespan(timeout, 0));	//original 
	session->setTimeout(Poco::Timespan(300L, 0));	//rox
...
void Connection::init(const std::string & connection_string) {
    /// connection_string - string of the form `DSN=Skm;UID=default;PWD=password`
...
void Connection::setDefaults() {
    ...
    if (server.empty())
        server = "localhost";
    if (port == 0)
        port = (proto == "https" ? 8443 : 8628);
    if (user.empty())
        user = "default";
    if (database.empty())
        database = "default";
    if (timeout == 0)
        timeout = 90;//rox  //original 30
}
