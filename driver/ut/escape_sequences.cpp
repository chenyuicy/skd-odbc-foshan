...
string processFunc(const StringView seq, Lexer & lex, int exeuteLevel) {
	...
	do{
		...
		if (
			(fn.type == Token::LCURLY && fn_curly_map.find(nexttoken.type) != fn_curly_map.end()) ||
			...
			while (param_count==0 || fn.type == Token::COMMA || fn.type == Token::AS) {	//循环与递归取参数	
				...
				param_count++;
				cout << "当前字符[" << param_count << "]：" << lex.Peek().literal.to_string() << endl;
				fn = lex.Consume();
				cout << "下一字符[" << param_count << "]：" << fn.literal.to_string() << "]:" << endl;
			}
			...
			for (int i = 0; i < param_count; i++) {
				if (i != 0)result += ",";
				cout << "param[" << param_count << "]:" << params[param_count] << endl;
			...
			if (nexttoken.type == Token::LPARENT) {
				...
				result += word;
				if(exeuteLevel>0)return result;
			}
			else {
				result += word;
			}
