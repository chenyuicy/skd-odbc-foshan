...
string processIdentOrFunction(const StringView seq, Lexer & lex) {
	...
	if (token.type == Token::LCURLY) {
		...
		const auto token_next = lex.Consume();
...
string processFunction(const StringView seq, Lexer & lex) {
	...
	if (fn.type == Token::SPACE) {
        ...
        if (lex.Consume().type == Token::LPARENT) {
...
string processFunc(const StringView seq, Lexer & lex, int exeuteLevel) {
	...
	string word ;
	//for (fn = lex.Consume(); fn.type != Token::EOS; fn=lex.Consume()) 
	...
	do{		
		...
		const Token	next = lex.LookAhead(1);
			...
			)
		{
			...
			function_name = (fn.type == Token::LCURLY)?fn_curly_map.at(next.type):function_map.at(fn.type);
			if (next.type==Token::FN)
			{
				level++;
				fn = lex.Consume();
				fn = lex.Consume();
			...
			lex.SetEmitSpaces(false);
			while (params.empty() || (fn.type != Token::RCURLY && fn.type != Token::RPARENT)) {	//循环与递归取参数	
				++exeuteLevel;
				params.push_back(processFunc(seq, lex, exeuteLevel));
				lex.SetEmitSpaces(false);
				fn = lex.LookAhead(1);
				lex.SetEmitSpaces(true);
				exeuteLevel--;
				//cout << "当前字符[" << param_count << "]：" << lex.Consume().literal.to_string() << endl;
				//fn = lex.ReadCurrent();
				//cout << "下一字符"  "：" << fn.literal.to_string() << "" << endl;
			}

			
			//result += function_name+"(";  //rox
			...
			if (fn.type == Token::LPARENT || fn.type == Token::LCURLY) {
				level++;
				word = (fn.type == Token::LCURLY) ? "(" : word;
				result += word;
			}
			else if (fn.type == Token::RPARENT || fn.type == Token::RCURLY) {
				level--;
				
				word = (fn.type == Token::RCURLY) ? ")" : word;
			...
			else if ((fn.type == Token::COMMA) && exeuteLevel > 0) {
				return result;
			}
			else {
				result += word;
		...
		lex.SetEmitSpaces(false);
		Token n = lex.LookAhead(1);
		lex.SetEmitSpaces(true);
		if (level == 0 && exeuteLevel > 0 &&
			((n.type == Token::COMMA) || (n.type == Token::RPARENT) || (n.type == Token::RCURLY))
			)
		{
			return result;
		}
